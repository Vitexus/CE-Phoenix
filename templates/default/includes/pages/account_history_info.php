...
if (defined('USE_ABRAFLEXI') && (constant('USE_ABRAFLEXI') == 'true')) {
    ...
    $oPage = new Ease\TWB4WebPage();

    Ease\Shared::webPage($oPage);

    Ease\TWB4Part::twBootstrapize();
    if (floatval($invoice->getDataValue('zbyvaUhradit'))) {
        $paymentInfo = new Ease\TWB4Panel(constant('HEADING_PAYMENT_METHOD'),
                'warning');
        ...
        $paymentInfo = new Ease\TWB4Panel(_('Payment method'), 'success');

        $paymentInfo->addItem(new Ease\Html\H3Tag(_('Already settled')));
        $paymentInfo->addItem(new Ease\TWB4\LinkButton('getpdf.php?evidence=faktura-vydana&id=' . $docId . '&embed=true',
                        _('PDF Invoice'), 'success'));
        $paymentInfo->addItem(new Ease\TWB4\LinkButton('getisdoc.php?evidence=faktura-vydana&id=' . $docId . '&embed=true',
                        _('ISDOC Invoice'), 'success'));
        $paymentInfo->addItem(new Ease\TWB4\LinkButton('getxls.php?evidence=faktura-vydana&id=' . $docId . '&embed=true',
                        _('XLS Invoice'), 'success'));
    }
} else {
    $paymentInfo = new Ease\TWB4\Panel(constant('HEADING_PAYMENT_METHOD'),
