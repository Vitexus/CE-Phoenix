...
class gpwebpay extends abstract_payment_module {

    const CONFIG_KEY_BASE = 'MODULE_PAYMENT_GPWEBPAY_';

    public $code = 'gpwebpay';
    ...
    public function __construct() {
        ...
        if (defined('MODULE_PAYMENT_GPWEBPAY_GATEWAY_SERVER') && (constant('MODULE_PAYMENT_GPWEBPAY_GATEWAY_SERVER') == 'Production')) {
    ...
    /**
     * Payment response title/message
     * 
     * @return array
     */
    ...
    function update_status() {
        ...
        if (($this->enabled == true) && ((int) constant('MODULE_PAYMENT_GPWEBPAY_ZONE') > 0)) {
            $check_flag = false;
            $check_query = tep_db_query("select zone_id from zones_to_geo_zones where geo_zone_id = '" . MODULE_PAYMENT_GPWEBPAY_ZONE . "' and zone_country_id = '" . $order->billing['country']['id'] . "' order by zone_id");
                ...
                for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
                    ...
                    if (isset($order->products[$i]['attributes'])) {
                        ...
                        for ($j = 0, $n2 = sizeof($order->products[$i]['attributes']); $j < $n2; $j++) {
                            ...
                            tep_db_perform('orders_products_attributes',
                                    $sql_data_array);
                            ...
                            if ((DOWNLOAD_ENABLED == 'true') && isset($attributes_values['products_attributes_filename']) && tep_not_null($attributes_values['products_attributes_filename'])) {
                                ...
                                tep_db_perform('orders_products_download',
                                        $sql_data_array);
                            }
                        }
                    }
...
//        CE-Phoenix/includes/modules/payment/gpwebpay.php
        ...
        require_once __DIR__ . '/../../../includes/apps/gpwebpay/gp-webpay-php-sdk/src/AdamStipak/Webpay/Api.php';
        ...
        require_once __DIR__ . '/../../../includes/apps/gpwebpay/gp-webpay-php-sdk/src/AdamStipak/Webpay/PaymentRequest.php';
        ...
        $request->setDescription(iconv('UTF-8', 'ASCII//TRANSLIT', $products_info));
...
//            $process_button_string          .= '<a class="btn btn-danger" href="checkout_success.php?'.http_build_query($fakeResponseData).'"> ByPass '.$e->getMessage().' </a>';
    ...
    function before_process() {
        ...
        for ($i = 0, $n = sizeof($order->products); $i < $n; $i++) {
            ...
            tep_db_query("update " . 'products' . " set products_ordered = products_ordered + " . sprintf('%d',
    ...
    function check() {
        if (!isset($this->_check)) {
            $check_query = tep_db_query("select configuration_value from configuration where configuration_key = 'MODULE_PAYMENT_GPWEBPAY_STATUS'");
    ...
    function keys() {
        ...
        return ['MODULE_PAYMENT_GPWEBPAY_STATUS', 'MODULE_PAYMENT_GPWEBPAY_GATEWAY_SERVER',
    ...
    function calcGpwebpayMd5Key($order) {
        ...
        $q = http_build_query(["merchant_id" => $order['merchant_id'],
...
//        $created_status_id = $this->set_order_status('Processing [gpwebpay]', true);
//        $sum_too_low_status_id = $this->set_order_status('Sum too low [gpwebpay]', true);
//        $completed_status_id = $this->set_order_status('Completed [gpwebpay]', true);
//
//        $sort_order = 0;
//
//
...
////        ('Flow Layout', 'MODULE_PAYMENT_GPWEBPAY_FLOW_LAYOUT', 'multi_page', 'Layout for the buyer flow', '6', '" . $sort_order++ . "', now())");
//
//        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values "
//                . "('Debug E-Mail Address', 'MODULE_PAYMENT_GPWEBPAY_DEBUG_EMAIL', '', 'All parameters of an Invalid IPN notification will be sent to this email address if one is entered.', '6', '" . $sort_order++ . "', now())");
//
...
//                . "('Set gpwebpay Acknowledged Order Status', 'MODULE_PAYMENT_GPWEBPAY_CREATE_ORDER_STATUS_ID', '" . $created_status_id . "', 'Set the status of orders made with this payment module to this value', '6', '" . $sort_order++ . "', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now())");
//        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values "
//                . "('Set gpwebpay sum too low Order Status', 'MODULE_PAYMENT_GPWEBPAY_SUM_TOO_LOW_ORDER_STATUS_ID', '" . $sum_too_low_status_id . "', 'Set the status of orders which are paid with insufficient fund (sum too low) to this value', '6', '" . $sort_order++ . "', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now())");
//        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values "
//                . "('Set gpwebpay Completed Order Status', 'MODULE_PAYMENT_GPWEBPAY_COMP_ORDER_STATUS_ID', '" . $completed_status_id . "', 'Set the status of orders which are confirmed as paid (approved) to this value', '6', '" . $sort_order++ . "', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now())");
//
...
////        tep_db_query("insert into ".TABLE_CONFIGURATION." (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('OpenSSL Location', 'MODULE_PAYMENT_GPWEBPAY_EWP_OPENSSL', '/usr/bin/openssl', 'The location of the openssl binary file.', '6', '4', now())");
        ...
        return [
            'MODULE_PAYMENT_GPWEBPAY_STATUS' => [
                'title' => _('Enable GP WebPay Module'),
                'value' => 'True',
                'desc' => 'Do you want to accept Card payments?',
                'set_func' => "tep_cfg_select_option(['True', 'False'], ",
            ],
            'MODULE_PAYMENT_GPWEBPAY_ZONE' => [
                'title' => _('Payment Zone'),
                'value' => '0',
                'desc' => _('If a zone is selected, only enable this payment method for that zone.'),
                'use_func' => 'tep_get_zone_class_title',
                'set_func' => 'tep_cfg_pull_down_zone_classes(',
            ],
            'MODULE_PAYMENT_GPWEBPAY_SORT_ORDER' => [
                'title' => _('Sort order of display.'),
                'value' => '0',
                'desc' => _('Sort order of display. Lowest is displayed first.'),
            ],
            'MODULE_PAYMENT_GPWEBPAY_ORDER_STATUS_ID' => [
                'title' => _('Set Order Status'),
                'value' => '0',
                'desc' => _('Set the status of orders made with GPWebPay module to this value'),
                'set_func' => 'tep_cfg_pull_down_order_statuses(',
                'use_func' => 'tep_get_order_status_name',
            ],
            'MODULE_PAYMENT_GPWEBPAY_GATEWAY_SERVER' => [
                'title' => _('Gateway Server'),
                'value' => _('Production'),
                'desc' => _('Use the testing or production gateway server for transactions'),
                'set_func' => 'tep_cfg_select_option([\'Production\', \'Test\'],'
            ],
            'MODULE_PAYMENT_GPWEBPAY_MERCHANT_ID' => [
                'title' => _('Your merchant id'),
                'value' => '00000000',
                'desc' => _('Your merchant unique identifier (supplied by gpwebpay)')
            ],
            'MODULE_PAYMENT_GPWEBPAY_SECRET_KEY' => [
                'title' => _('Your secret key'),
                'desc' => _('Your secret key (supplied by gpwebpay)')
            ],
            'MODULE_PAYMENT_GPWEBPAY_SECRET_KEY_PASSWORD' => [
                'title' => _('Your secret key password'),
                'desc' => _('Your secret key password (supplied by gpwebpay)')
            ],
            'MODULE_PAYMENT_GPWEBPAY_PUBLIC_KEY' => [
                'title' => _('Your public key'),
                'desc' => _('Your public key (supplied by gpwebpay)')
            ],
            'MODULE_PAYMENT_GPWEBPAY_DECREASE_STOCK_ON_CREATION' => [
                'title' => _('Decrease stock on payment creation'),
                'value' => 'False',
                'desc' => _('Do you want to decrease stock upon payment creation?'),
                'set_func' => "tep_cfg_select_option(['True', 'False'],"
            ]
        ];
    }
