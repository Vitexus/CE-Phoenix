...
$xsell_query = tep_db_query("select distinct p.products_id, p.products_image, pd.products_name from 
 articles_xsell ax left join 
 products p on ax.xsell_id = p.products_id left join 
 products_description pd on p.products_id = pd.products_id
 where ax.articles_id = '" . (int)$_GET['articles_id'] . "'  and pd.language_id = '" . (int)$languages_id . "' and p.products_status = '1' order by ax.sort_order asc limit " . MAX_DISPLAY_ARTICLES_XSELL);
...
<?php
      ...
      while ($xsell = tep_db_fetch_array($xsell_query)) {
        $path = 'images/' . $xsell['products_image'];
        $xsell['products_name'] = tep_get_products_name($xsell['products_id']);
        
        echo '<div class="articleBox-child">';
        if (tep_not_null($xsell['products_image']) && file_exists($path)) { 
        ...
        echo '<a href="' . tep_href_link('product_info.php', 'products_id=' . $xsell['products_id']) . '">' . addslashes($xsell['products_name']) . '</a></div>';
      }
      echo '</div>';
?>
<!-- xsell_articles_eof //-->
